...
class LeftAction(UpdateNew):     # 左移不需要对矩阵操作，基础类toSequence即根据左移来写
    ...
    def handleData(self):
        matrix = self.matrix.copy()  # 直接获得一份矩阵的复制
        newmatrix = self.toSequence(matrix)
        return newmatrix, self.score


class RightAction(UpdateNew):  # 右移是左移的逆向操作
    ...
    def handleData(self):
        matrix = self.matrix.copy()[:, ::-1]   # 先对矩阵每行逆序
        newmatrix = self.toSequence(matrix)    # 左移操作
        return newmatrix[:, ::-1], self.score  # 再对列表逆序操作


class UpAction(UpdateNew):   # 向上是左移的转置
    ...
    def handleData(self):
        matrix = self.matrix.copy().T          # 先对矩阵转置
        newmatrix = self.toSequence(matrix)    # 左移
        return newmatrix.T, self.score         # 再次转置


class DownAction(UpdateNew):  # 向下操作
    ...
    def handleData(self):
        matrix = self.matrix.copy()[::-1].T   # 先对列表逆序后转置
        newmatrix = self.toSequence(matrix)   # 左移
        return newmatrix.T[::-1], self.score  # 转置后逆序
...
class GameInit(object):
    ...
    @classmethod   # 描绘标题函数，内部调用了描绘方块函数
    ...
    def drawBlock(screen, row, column, color, blocknum):
        font = pygame.font.SysFont('stxingkai', 80)  # 设置字体
    ...
    def gameOver(matrix):
        ...
        a, b = testmatrix.shape    # 用a，b保存数组的行列数
        # 判断游戏结束条件，矩阵不为0且每行每列不为0
        for i in range(a):
            for j in range(b - 1):
        ...
        for i in range(b):
            for j in range(a - 1):
                if testmatrix[j][i] == testmatrix[j + 1][i]:  # 每列有相同的数
                    print('游戏没有结束')
                    return False
        print('游戏结束')
        return True
...
def main():
    pygame.init()     #初始化
    screen = pygame.display.set_mode(Screen_Size, 0, 0)  # 屏幕设置
